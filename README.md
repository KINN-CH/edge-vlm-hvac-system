# 엣지 VLM 및 IoT 융합 기반 지능형 공조(HVAC) 제어 시스템

## 1. 프로젝트 요약 (Executive Summary)
본 프로젝트는 소형 임베디드 보드(NVIDIA Jetson)의 제한된 리소스 내에서 시각-언어 모델(VLM)을 최적화하여 구동하는 **스마트 공조 제어 시스템**을 개발합니다. 카메라 영상 및 AI 추론(VLM·YOLO·PMV 계산)은 기기 내부에서 온디바이스로 처리되어 카메라 이미지의 외부 유출을 차단합니다. 실외 기상 정보는 기상청·에어코리아 API를 통해 수집합니다. 단순한 온도 센서 의존을 넘어, 재실자의 **'열 쾌적성(PMV)'을 선제적으로 예측**하여 에너지를 절감하는 산업용 **레트로핏(Retrofit) 솔루션**을 제안합니다.

---

## 2. 프로젝트 목표 (Project Objectives)
- **온디바이스 AI 파이프라인 구축:** 카메라 이미지로부터 공간의 복합적인 맥락(Context)을 파악하는 VLM을 엣지 디바이스에 포팅하고, **INT4 양자화 및 TensorRT 가속화**를 통해 실시간 추론 성능을 확보합니다. 카메라 영상은 기기 외부로 전송되지 않으며, 실외 환경 데이터(기온·습도·대기질)는 기상청·에어코리아 API로 보강합니다.
...
### ① 온디바이스 AI 추론 (Privacy-Preserving)
- **차별점:** 카메라 이미지는 외부 서버로 전송되지 않으며, VLM·YOLO·PMV 계산 등 모든 AI 추론이 기기 내부(온디바이스)에서 처리됩니다. 실외 기온·습도·대기질은 기상청·에어코리아 API를 통해 수집합니다.
- **기대효과:** AI 추론 지연(Latency) 없는 즉각 처리 및 보안 구역에서도 카메라 영상 유출 제로(0)를 보장합니다.
